...
unsigned int nStakeMinAge       = 1 * 24 * 60 * 60; // 1 days min stake age
...
const string strMessageMagic = "IDOL Signed Message:\n";
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modsum=%u, modifier=0x%016" PRIx64, pindexNew->nHeight, pindexNew->nStakeModifierChecksum, nStakeModifier);
...
bool CBlock::AcceptBlock()
{
    ...
    if (nVersion < 7 && nHeight >= HARDFORK_BLOCK)
        return DoS(100, error("AcceptBlock() : reject old nVersion Block %d", nVersion));
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "IDOL", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Cheer idols, Interact more closely with idols, Support idols more directly";
        CTransaction txNew;
        txNew.nTime = 1538040072;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1538040072;
        block.nVersion = 1;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3677645;
        if(fTestNet)
        {
            block.nNonce   = 327733;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x2e953947cd107664f5a0bdc5ea99c3ce4537ed66cf04eb46eac7284e3b790a2e"));
        block.print();
