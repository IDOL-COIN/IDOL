...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_CODESEPARAATR          : return "OP_CODESEPARAATR";
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_SIZE:
                {
                    ...
                    if (stack.size() < 1)
                        return false;
                    CBigNum bn((int32_t)stacktop(-1).size());
                    stack.push_back(bn.getvch());
                }
                break;
...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    scriptCode.FindAndDelete(CScript(OP_CODESEPARAATR));
